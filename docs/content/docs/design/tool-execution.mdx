...
graph TD
    subgraph "Agent"
        A[Brain] -- "Generates Tool Call" --> B(Agent Core)
    end
    ...
    B -- "Delegates Call" --> C
    C -- "Validates against Schema" --> D
    C -- "Validation FAILED" --> B
    B -- "Sends Error to LLM for Correction" --> A

    C -- "Validation PASSED<br/>Dispatches for Execution" --> E
    ...
    E -- "Returns Structured Result" --> C
    C -- "Forwards Result" --> B
    B -- "Formats for LLM &<br/>Adds to Context" --> A
```
