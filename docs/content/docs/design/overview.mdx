...
- **Core Framework**: The central orchestration layer containing `TaskExecutor`, `Orchestrator`, `Agent`, and `Brain` components. Core imports FROM subsystems rather than the other way around, implementing true dependency inversion.
...
- **Task Executor**: Owns the end-to-end lifecycle of a single task. It acts as the primary workflow engine, responsible for provisioning the workspace, managing the overall task state, deciding which agent to invoke in each turn of the conversation, and providing secure tool execution capabilities through its integrated `ToolManager`.
- **Orchestrator**: Acts as a dedicated service for intelligent agent routing and coordination. It makes decisions about which agent should execute next and when tasks should be completed, but does **not** handle tool execution. The Orchestrator focuses purely on multi-agent coordination logic.
- **Agent**: Encapsulates a specialised role (e.g., _researcher_, _writer_). It receives control from the `Task Executor`, reasons with its private **Brain**, and can execute tools directly through the `ToolManager` provided by the Task Executor. Agents no longer need to request tool execution from the Orchestrator.
...
sequenceDiagram
    ...
    loop Autonomous Loop
        TX->>AG: invoke agent
        AG->>TM: execute tool calls
        TM-->>AG: return tool results
        AG-->>TX: return turn response
        TX->>OR: decide next step
        OR-->>TX: routing decision
    end
    TX-->>C: final result
```
...
- **Simplified Agent Interface**: Agents no longer need to communicate with the Orchestrator for tool execution, reducing complexity and improving performance.
...
- **Custom Tools**: Developers can add their own tools by decorating Python functions. These are registered with the task's ToolManager and exposed to agents in the same way as builtin tools.
