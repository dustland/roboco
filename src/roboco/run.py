...
def start():
    """Start the Roboco API server with integrated observability."""
    print("🤖 Starting Roboco API Server (Integrated Mode)")
    print("=" * 50)
    ...
    try:
        ...
        from roboco.server.api import app
        import uvicorn
...
def monitor(data_dir: Optional[str] = None):
    ...
    try:
        ...
        if monitor.is_integrated:
            print("⚠️  Warning: Detected integrated mode. Consider using 'roboco start' instead.")
        ...
        while True:
            try:
                ...
                if cmd == "quit" or cmd == "exit":
                    ...
                    filename = f"roboco_observability_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
