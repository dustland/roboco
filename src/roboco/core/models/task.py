...
class Task(BaseModel):
    ...
    created_at: str = Field(default_factory=lambda: datetime.now().isoformat(), description="When the task was created")
    updated_at: str = Field(default_factory=lambda: datetime.now().isoformat(), description="When the task was last updated")
    completed_at: Optional[str] = Field(None, description="When the task was completed")
